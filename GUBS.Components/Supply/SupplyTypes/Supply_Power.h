...
{
	class Supply_Power : public _Supply
	{
	public:
		Supply_Power() : _Supply() { _Type = SupplyType::POWER; }

		Supply_Power(const _Supply& def) :_Supply(def) { _Type = SupplyType::POWER; }
		Supply_Power(Supply_Power&& def) = default;

		Supply_Power(unsigned long key, const _Supply& def) :_Supply(key, def) { _Type = SupplyType::POWER; }
